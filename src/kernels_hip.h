#ifndef KERNELS_HIP_H
#define KERNELS_HIP_H
...
static __global__ void beam_search(
    ...
    // scores are NTC: successive timesteps for a batch element are one C-stride apart,
    // and the batch element's block is at chunk * T * C.
    ...
    if (tid == 0) {
        ...
        for (size_t state = 0, beam_element = 0; state < num_states && beam_element < MAX_BEAM_WIDTH; state++) {
            if (bwd_NTC[state] >= beam_init_threshold) {
                ...
                prev_scores[beam_element] = 0.0f;
                ++beam_element;
                current_beam_width = beam_element;
            }
        }
    }
    __syncthreads();
