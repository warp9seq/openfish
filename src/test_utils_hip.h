...
static inline void write_gpubuf_hip(
    ...
    ret = hipMemcpy(bwd_NTC, gpubuf->bwd_NTC, sizeof(float) * batch_size * (n_timesteps + 1) * num_states, hipMemcpyDeviceToHost);
    checkHipError(); HIP_CHECK(ret);
    ret = hipMemcpy(post_NTC, gpubuf->post_NTC, sizeof(float) * batch_size * (n_timesteps + 1) * num_states, hipMemcpyDeviceToHost);
    checkHipError(); HIP_CHECK(ret);
    ret = hipMemcpy(states, gpubuf->states, sizeof(state_t) * batch_size * n_timesteps, hipMemcpyDeviceToHost);
    checkHipError(); HIP_CHECK(ret);
    ret = hipMemcpy(total_probs, gpubuf->total_probs, sizeof(float) * batch_size * n_timesteps, hipMemcpyDeviceToHost);
    checkHipError(); HIP_CHECK(ret);
    ret = hipMemcpy(qual_data, gpubuf->qual_data, sizeof(float) * batch_size * n_timesteps * NUM_BASES, hipMemcpyDeviceToHost);
    checkHipError(); HIP_CHECK(ret);
    ret = hipMemcpy(base_probs, gpubuf->base_probs, sizeof(float) * batch_size * n_timesteps, hipMemcpyDeviceToHost);
    checkHipError(); HIP_CHECK(ret);
